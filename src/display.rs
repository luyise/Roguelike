...
pub fn clean_logs() -> Result<()> {
    ...
    for j in (N_HEIGHT + 2)..(DISP_HEIGHT + N_HEIGHT + 2) {
        execute!(stdout(), cursor::MoveTo(1, j), Print(&empty_strip))?
    }
    Ok(())
}
                ...
                None => {
                    for l in 0..9 {
                        execute!(
                            stdout(),
                            cursor::MoveTo(
                                N_WIDTH + 2 + 15 + 1 + (x as u16) * (20 + 1),
                                1 + 3 + 1 + (y as u16) * (9 + 1) + l
                            ),
                            Print(&empty_strip)
                        )?
                    }
...
pub fn print_screen_background() -> Result<()> {
    ...
    grid.draw_line(1 + N_WIDTH as usize, 0, 1 + N_WIDTH as usize, sh - 1, 1)
        .unwrap();
    grid.draw_line(
        0,
        1 + N_HEIGHT as usize,
    ...
    execute!(stdout(), SetForegroundColor(SCREEN_BOUNDARIES_CLR),)?;
