...
fn main() -> Result<()> {
    ...
    'running: loop {
        ...
        if event::poll(Duration::from_millis(500)).unwrap() {
            ...
            match event::read().unwrap() {
                Event::Key(event) => match event.code {
                    KeyCode::Left | KeyCode::Char('q') => {         // Deplacement
                        gs.move_player(-1, 0)
                    },
        ...
        execute!(stdout(),
            cursor::MoveTo(1, N_HEIGHT+3),
        )?;
        println!("DEBUG: player.pos : {} {}      ", gs.player.pos.x, gs.player.pos.y);
        execute!(stdout(),
            cursor::MoveTo(1, N_HEIGHT+4),
        )?;
        println!("DEBUG: screen.pos : {} {}      ", gs.screen_pos.x, gs.screen_pos.y);
...
fn refresh_screen(screen_state_already_displayed: ScreenState, screen_state_to_display : &ScreenState) -> Result<()> {
    for j in 0..N_HEIGHT {
        for i in 0..N_WIDTH {
            if screen_state_already_displayed.grid[i as usize][j as usize] != screen_state_to_display.grid[i as usize][j as usize] {
...
fn print_screen_background() -> Result<()> {
    ...
    execute!(stdout(),                                  // Coin Haut-gauche
        cursor::MoveTo(0, 0),
        SetForegroundColor(SCREEN_BOUNDARIES_CLR),
        Print('\u{2554}'.to_string()),
    )?;
    for _ in 1..=(N_WIDTH) {                            // Bord supérieur
        execute!(stdout(),
            Print('\u{2550}'.to_string()),
        )?
    };
    execute!(stdout(),
            Print('\u{2566}'.to_string()),
    )?;
    ...
    for j in 1..=(N_HEIGHT) {                              // Bordures verticales
        execute!(stdout(),
            cursor::MoveTo(0, j),
    ...
    execute!(stdout(),                          // Bord horizontal intermédiaire
    ...
    execute!(stdout(),                          // Bord horizontal inférieure
